...
for i in range(5):
    ...
    # file_path for pa 'precision' BUT THIS HAS TO BE CODED FOR ALL DATASETS.  This is a draft.
    
    file_path = f"processed_data/PA/{precision}_fold_{i}.csv"

    model_path = f"models/{dataset}/{split}/{dataset}_{train_folds}_{i}.pth"

    # embedding_path for pa 'precision' BUT THIS HAS TO BE CODED FOR ALL DATASETS.  This is a draft.
    embedding_path = f"models/PA/{precision}_embeddings.pkl"
    ...
    with torch.no_grad():
        ...
        df.to_csv(f'results/{precision}_{dataset}_{split}_{train_folds}_{i}.csv', index=False)
